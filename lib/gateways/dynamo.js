      ...
      if(data.uploads && Object.keys(data.uploads).length > 0){
        Object.keys(data.uploads).forEach(uploadId => {
          if(data.uploads[uploadId].fileName){
            ...
            updateExpressionItems.push(`uploads.#key${uploadId} = :val${uploadId}`);
            ExpressionAttributeNames[`#key${uploadId}`] = uploadId;
            ExpressionAttributeValues[`:val${uploadId}`] = data.uploads[uploadId];
          }else if(data.uploads[uploadId].title && data.uploads[uploadId].title !== ''){
            // Update an existing upload
            updateExpressionItems.push(`uploads.#key${uploadId}.title = :val${uploadId}`);
            ExpressionAttributeNames[`#key${uploadId}`] = uploadId;
            ExpressionAttributeValues[`:val${uploadId}`] = data.uploads[uploadId].title;
          }
        })
      }
