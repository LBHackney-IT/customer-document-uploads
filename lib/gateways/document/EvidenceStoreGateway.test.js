  ...
  describe('createUploadUrl', () => {
    ...
    beforeEach(() => {
      scope = nock('http://localhost:5050')
        .post('/metadata', { dropboxId: expectedDropboxId, ...metadata })
        .reply(201, {
          documentId: 'ajs873',
    ...
    it('calls Evidence Store API with the dropbox id', async () => {
      await gateway.createUploadUrl({ dropboxId: expectedDropboxId, metadata });
      expect(scope.isDone()).toBe(true);
    });

    it('returns correct url and fields', async () => {
      const result = await gateway.createUploadUrl({
        dropboxId: expectedDropboxId,
        metadata
      });
    ...
    it('throws an error if API call fails', async () => {
      ...
      await expect(() =>
        gateway.createUploadUrl({ dropboxId: '123456' })
      ).rejects.toThrow();
  ...
  describe('getByDropboxId', () => {
    it('returns a list of documents within a dropbox', async () => {
      ...
      nock('http://localhost:5050')
        .post('/search', { metadata: { dropboxId: expectedDropboxId } })
        .reply(200, {
          documents: [
            {
              metadata: {
