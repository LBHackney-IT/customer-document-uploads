...
describe('gateways/request/DynamoDB', () => {
  ...
  describe('save', () => {
    ...
    beforeEach(() => {
      ...
      request = DocumentRequest.create(metadata);
  ...
  describe('update', () => {
    ...
    beforeEach(() => {
      options.client.update.mockImplementation((_, cb) => cb(null, {}));
    ...
    it('updates a request', async () => {
      ...
      await gateway.update(request);

      expect(options.client.update).toHaveBeenCalledWith(
        expect.objectContaining({
          TableName: options.configuration.tables.requests,
