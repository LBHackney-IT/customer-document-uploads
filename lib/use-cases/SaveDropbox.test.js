...
describe('use-case/save dropbox', () => {
  ...
  describe('with a valid dropbox id', () => {
    ...
    it('archives the dropbox if requestId is set', async () => {
      options.gateways.dropbox.get.mockImplementation(() => {
        return Promise.resolve({ id: '1', requestId: 'req1', archived: null });
      });
      await saveDropbox('1', {});
    ...
    it('does not archive the dropbox if requestId is not set', async () => {
      options.gateways.dropbox.get.mockImplementation(() => {
        return Promise.resolve({ id: '1', archived: null });
      });
      await saveDropbox('1', {});
      expect(options.gateways.dropbox.save).toHaveBeenCalledWith(
        expect.objectContaining({ archived: null })
      );
    });
